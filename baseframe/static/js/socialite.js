  ...
  var socialite = {
    ...
    getElements: function (context, cn) {
      ...
      var i = 0,
        ...
        all = gcn
          ? context.getElementsByClassName(cn)
    ...
    appendNetwork: function (network) {
      ...
      if (
        typeof network.append === 'function' &&
        network.append(network) === false
      ) {
      ...
      if (network.script) {
        ...
        network.el.onload = network.el.onreadystatechange = function () {
          if (rstate.test(network.el.readyState || '')) {
            ...
            network.loaded = true;
            if (
              typeof network.onload === 'function' &&
              network.onload(network) === false
            ) {
              return;
            }
            socialite.activateAll(network);
    ...
    createInstance: function (el, widget) {
      ...
      if (widget.process !== undefined) {
        proceed =
          typeof widget.process === 'function'
            ? widget.process(instance)
            : false;
      }
      if (proceed) {
      ...
      instance.el.className =
        'socialite ' + widget.name + ' socialite-instance';
      return instance;
    },
    ...
    processInstance: function (instance) {
      ...
      if (
        ...
        !el.getAttribute('data-default-href')
      ) {
    ...
    load: function (context, el, w, onload, process) {
      ...
      context =
        context && typeof context === 'object' && context.nodeType === 1
          ? context
          : document;
      ...
      if (!el || typeof el !== 'object') {
        socialite.load(
          context,
          socialite.getElements(context, 'socialite'),
          w,
          onload,
          process
        );
...
(function (window, document, Socialite, undefined) {
  ...
  Socialite.network('facebook', {
    ...
    append: function (network) {
      ...
      network.script.src = network.script.src.replace(
        '{{language}}',
        settings.lang
      );
  ...
  var twitterInit = function (instance) {
    ...
    el.setAttribute(
      'data-lang',
      instance.el.getAttribute('data-lang') || Socialite.settings.twitter.lang
    );
    instance.el.appendChild(el);
  };
  ...
  Socialite.widget('twitter', 'embed', {
    process: function (instance) {
      ...
      if (!instance.innerEl.getAttribute('data-lang')) {
        instance.innerEl.setAttribute(
          'data-lang',
          Socialite.settings.twitter.lang
        );
      }
      instance.el = document.createElement('div');
