...
(function ($) {
  ...
  var load = function (templateName) {
    ...
    if (typeof cache[templateName] === 'undefined') {
      ...
      var hoganTemplateName = domTemplateName
        .slice(domTemplateName.lastIndexOf('-') + 1)
        .replace('.mustache', '');
      if (document.getElementById(domTemplateName)) {
        ...
        cache[templateName] = $('<div />')
          .html($.trim($(document.getElementById(domTemplateName)).html()))
          .text();
  ...
  var render = function (templateName, context) {
    ...
    $.error(
      'Must have either Hogan.js or Mustache.js to load string templates'
    );
  };
